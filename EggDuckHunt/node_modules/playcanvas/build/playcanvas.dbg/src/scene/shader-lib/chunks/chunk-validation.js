import { CHUNKAPI_1_57, CHUNKAPI_1_60, CHUNKAPI_1_55, CHUNKAPI_1_62, CHUNKAPI_1_65, CHUNKAPI_1_70, CHUNKAPI_1_51, CHUNKAPI_1_56, CHUNKAPI_2_1, CHUNKAPI_2_3, CHUNKAPI_2_5 } from '../../../platform/graphics/constants.js';
...
var semverLess = (a, b)=>{
    var aver = a.split('.').map((t)=>parseInt(t, 10));
    var bver = b.split('.').map((t)=>parseInt(t, 10));
    return aver[0] < bver[0] || aver[0] === bver[0] && aver[1] < bver[1];
};
// validate user chunks
var validateUserChunks = (userChunks)=>{
    var userAPIVersion = userChunks.APIVersion;
    for(var chunkName in userChunks){
        if (chunkName === 'APIVersion') {
            continue;
        }
        if (!shaderChunks.hasOwnProperty(chunkName)) {
            var removedVersion = removedChunks[chunkName];
            if (removedVersion) {
            ...
            var engineAPIVersion = chunkVersions[chunkName];
            var chunkIsOutdated = engineAPIVersion && (!userAPIVersion || semverLess(userAPIVersion, engineAPIVersion));
            if (chunkIsOutdated) {
